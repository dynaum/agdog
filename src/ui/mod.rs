...
pub fn render(frame: &mut Frame, app: &App) {
    ...
    cpu::render(frame, v[1], app);
    gpus::render(frame, v[2], app);
    agents::render(frame, v[3], app);

    let bottom =
        Layout::horizontal([Constraint::Percentage(55), Constraint::Percentage(45)]).split(v[4]);
    detail::render(frame, bottom[0], app);
    alerts::render(frame, bottom[1], app);

    footer::render(frame, v[5], app);
}
